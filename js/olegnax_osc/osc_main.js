...
OnestepcheckoutBox.prototype = {
    ...
    resizeBox: function() 
    {
		var pppIframe = this.contentContainer.select('#hss-iframe').first();		
		if(pppIframe != undefined)
		{
			var pppIframeWidth = 425;
			var pppIframeHeight = pppIframe.readAttribute('height');
			var boxTop = (document.viewport.getHeight() - pppIframeHeight) / 2;
			if (boxTop < 25)
			{
				boxTop = 25;
				pppIframeHeight = document.viewport.getHeight() - 50;
			}
			var boxLeft = (document.viewport.getWidth() - pppIframeWidth) / 2;
			pppIframe.writeAttribute('scrolling', 'yes');
			pppIframe.setStyle({ width: pppIframeWidth + 'px', height: pppIframeHeight + 'px' });
			var warningLabelHeight = this.contentContainer.select('#iframe-warning').first().getHeight();
			this.contentContainer.setStyle({ width: pppIframeWidth + 'px', height: pppIframeHeight + warningLabelHeight + 'px' });
